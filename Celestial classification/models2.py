...
train_data_path = "./data/oversampling.csv"
test_data_path = "./data/test_nofiber.csv"
...
# xgboost
#xgb_clf = XGBClassifier(n_estimators = 10000, n_jobs=-1, random_state=random_seed, tree_method='gpu_exact')
...
submission.to_csv('./results/over_xg_boost.csv', index=True)

# catboost
#cat_clf = CatBoostClassifier(iterations = 10000, random_state = random_seed, task_type="GPU")
cat_clf = CatBoostClassifier(iterations = 1000, random_state = random_seed)
...
submission.to_csv('./results/over_cat_boost2.csv', index=True)

# lgbm
#lgbm_clf = LGBMClassifier(n_estimators = 10000, n_jobs=-1, random_state = random_seed, device = 'gpu')
...
submission.to_csv('./results/over_light_gbm.csv', index=True)
